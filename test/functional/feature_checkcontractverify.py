...
class P2TR:
    ...
    def get_script(self, clause_name: str):
        for name, script in flatten_scripts(self.scripts):
            if name == clause_name:
                return script
...
class AugmentedP2TR:
    ...
    def get_script(self, clause_name: str):
        for name, script in flatten_scripts(self.get_scripts()):
            if name == clause_name:
                return script
        raise ValueError(f"Clause {clause_name} not found")
    ...
    *,
    ...
    for inp in inputs:
        ...
        if isinstance(inp.contract, AugmentedP2TR):
            assert inp.data is not None and len(inp.data) == 32
