...
static void ValidateCheckInputsForAllFlags(const CTransaction &tx, script_verify_flags failing_flags, bool add_to_cache, CCoinsViewCache& active_coins_tip, ValidationCache& validation_cache) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    for (int count = 0; count < 10000; ++count) {
        ...
        bool ret = CheckInputScripts(tx, state, &active_coins_tip, test_flags, true, add_to_cache, txdata, validation_cache, ccv_data, nullptr);
        ...
        if (ret && add_to_cache) {
            ...
            CcvTransactionExecutionData ccv_data2{tx.vin.size()};
            BOOST_CHECK(CheckInputScripts(tx, state, &active_coins_tip, test_flags, true, add_to_cache, txdata, validation_cache, ccv_data2, &scriptchecks));
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    // spend_tx is invalid according to DERSIG
    {
        ...
        CcvTransactionExecutionData ccv_data{spend_tx.vin.size()};

        BOOST_CHECK(!CheckInputScripts(CTransaction(spend_tx), state, &m_node.chainman->ActiveChainstate().CoinsTip(), SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG, true, true, ptd_spend_tx, m_node.chainman->m_validation_cache, ccv_data, nullptr));

        // If we call again asking for scriptchecks (as happens in
        // ConnectBlock), we should add a script check object for this -- we're
        // not caching invalidity (if that changes, delete this test case).
        std::vector<CScriptCheck> scriptchecks;
        CcvTransactionExecutionData ccv_data2{spend_tx.vin.size()};
        BOOST_CHECK(CheckInputScripts(CTransaction(spend_tx), state, &m_node.chainman->ActiveChainstate().CoinsTip(), SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG, true, true, ptd_spend_tx, m_node.chainman->m_validation_cache, ccv_data2, &scriptchecks));
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        TxValidationState state;
        PrecomputedTransactionData txdata;
        CcvTransactionExecutionData ccv_data{invalid_with_cltv_tx.vin.size()};
        BOOST_CHECK(CheckInputScripts(CTransaction(invalid_with_cltv_tx), state, m_node.chainman->ActiveChainstate().CoinsTip(), SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY, true, true, txdata, m_node.chainman->m_validation_cache, ccv_data, nullptr));
    }
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        TxValidationState state;
        PrecomputedTransactionData txdata;
        CcvTransactionExecutionData ccv_data{invalid_with_csv_tx.vin.size()};
        BOOST_CHECK(CheckInputScripts(CTransaction(invalid_with_csv_tx), state, &m_node.chainman->ActiveChainstate().CoinsTip(), SCRIPT_VERIFY_CHECKSEQUENCEVERIFY, true, true, txdata, m_node.chainman->m_validation_cache, ccv_data, nullptr));
    }
...

    {
        ...
        BOOST_CHECK(!CheckInputScripts(CTransaction(tx), state, &m_node.chainman->ActiveChainstate().CoinsTip(), SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, true, true, txdata, m_node.chainman->m_validation_cache, ccv_data, nullptr));

        std::vector<CScriptCheck> scriptchecks;
        // Make sure this transaction was not cached (ie because the first
        // input was valid)
        CcvTransactionExecutionData ccv_data2{tx.vin.size()};
        BOOST_CHECK(CheckInputScripts(CTransaction(tx), state, &m_node.chainman->ActiveChainstate().CoinsTip(), SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, true, true, txdata, m_node.chainman->m_validation_cache, ccv_data2, &scriptchecks));
