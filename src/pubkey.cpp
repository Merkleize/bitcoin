...
{
    // With no tweaks, CCV only requires the serialized x-only keys to match. In
    // particular, neither key needs to represent a curve point in this case.
    if (data.empty() && merkle_root == nullptr) return *this == naked_key;
